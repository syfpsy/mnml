...
| F7 | DOM focus pass fired before window had OS focus (v0.2.13–v0.3.7) | Single attempt at a fixed delay raced the OS focus grant; later versions used an 8-step staircase that was expensive. | Main owns focus via `FOCUS_SEARCH_SCRIPT` + `scheduleSearchFocusVerification` with short bursts per path (e.g. show: `[0, 80, 200]`); renderer keeps one 120 ms backup only. Stops when `document.hasFocus()` **and** `document.activeElement === input` (`focusRunId` invalidates stale retries). | Don't add a second focus staircase in the renderer. Keep verification paired with `document.hasFocus()`. |
...
| O62 | Panel opened but unresponsive / later summons dead after unresponsive recovery — `safeReloadRenderer()` cleared `rendererReady` while readiness was only set in `once("did-finish-load")`, so it never re-armed; visible re-summon also skipped `setIgnoreMouseEvents(false)`, leaving a click-through ghost window. | `on("did-finish-load")` re-arms ready every load; ignore-mouse only while hidden; every show path enables input; summon self-heal + tray Reload panel / Open log. |
...
9. **Tray apps must survive renderer death.** If the main process only logs a crash and keeps the tray icon, users perceive "mnml is running but won't open." Recover by recreating/reloading the `BrowserWindow`, and never trust a logical visibility flag without reconciling against `win.isVisible()`. Readiness flags cleared by reload must be re-armed on every `did-finish-load` (`on`, not `once`).
...
11. **Hidden click-through (`setIgnoreMouseEvents(true)`) must be cleared on every reveal path.** A visible panel that still ignores mouse events looks "open but dead." Always enable input on show — including the already-visible re-summon branch and post-reload-while-visible. Encode these in `check:summon` so they don't regress.